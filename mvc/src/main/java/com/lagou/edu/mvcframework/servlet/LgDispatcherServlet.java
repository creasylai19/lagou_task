        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(aClass.isAnnotationPresent(Security.class)){
                    Security securityOnClass = aClass.getAnnotation(Security.class);
                    for (String s : securityOnClass.value()) {
                        usernames.add(s);
                    }
                }
                if(method.isAnnotationPresent(Security.class)){
                    Security securityOnMathod = method.getAnnotation(Security.class);
                    for (String s : securityOnMathod.value()) {
                        usernames.add(s);
                    }
                }
                handler.setAccessUsers(usernames);
...
//        method.invoke() //
        ...
        Set<String> accessUsers = handler.getAccessUsers();
        if (null != accessUsers && accessUsers.size() > 0) {
